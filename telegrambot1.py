...
def next_question(update, context):
    ...
    if current_question >= len(questions):
        ...
        # Check if there was a previous question
        if current_question > 0:
            # Edit the previous question message with the new question
...
def password_input(update, context):
    ...
    if context.user_data['login'] in authorized_users and authorized_users[context.user_data['login']] == password:
        menu(update, context)
        return QUIZ  # Enter the QUIZ state after successful login
    else:
